...
class _QrDepositScreenState extends State<QrDepositScreen>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      // Rimostro il loader per evitare di mostrare la UI sbagliata
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text('Approvare il versamento in contanti di'),
            const Text('€ 50,00',
                style: TextStyle(
                    fontSize: 22,
      ...
      body: _isCheckingPermission
          ...
          : _hasPermission
              ? Stack(
                  children: [
                    MobileScanner(
                        controller: _scannerCtrl, onDetect: _onDetect),

                    // Overlay scuro con buco centrale
                    ColorFiltered(
...
class _ScannerCornerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(
        Offset(tr.dx - radius, tr.dy), Offset(tr.dx - length, tr.dy), paint);
    canvas.drawLine(Offset(tr.dx, tr.dy + radius),
        Offset(tr.dx, tr.dy + length), paint);
    canvas.drawArc(
    ...
    canvas.drawLine(
        Offset(bl.dx + radius, bl.dy), Offset(bl.dx + length, bl.dy), paint);
    canvas.drawLine(Offset(bl.dx, bl.dy - radius),
        Offset(bl.dx, bl.dy - length), paint);
    canvas.drawArc(
    ...
    canvas.drawLine(
        Offset(br.dx - radius, br.dy), Offset(br.dx - length, br.dy), paint);
    canvas.drawLine(Offset(br.dx, br.dy - radius),
        Offset(br.dx, br.dy - length), paint);
    canvas.drawArc(
        Rect.fromLTWH(br.dx - radius * 2, br.dy - radius * 2, radius * 2, radius * 2),
        0, 1.57, false, paint);
  }
